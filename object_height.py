...
# ── Calibration Inputs ─────────────────────────────────────────────────────────
""" This is the FOV of the time of flight sensor. Default to 65 degrees"""
TOF_FOV = 55
...
def export_stl(
    ...
    diagonal_mm = 2 * standoff * np.tan(np.deg2rad(TOF_FOV)/2)
    cell_mm = diagonal_mm / (8.0 * np.sqrt(2.0))
    print("with a FOV of:", TOF_FOV, " the total diagonal is: ", diagonal_mm)
    print("with a FOV of:", TOF_FOV, " the zone length is: ", cell_mm)

    grid = ZoneHeightGrid(zone_heights)

    # Build 8x8 height matrix — 0 height gets floor tile
    heights = np.zeros((8, 8), dtype=np.float32)
    for row, col, h in grid:
        heights[row, col] = h if h > 0.0 else _STL_FLOOR_HEIGHT_MM

    triangles: list[list[np.ndarray]] = []

    # --- Single floor quad covering the entire 8x8 grid ---
    gx0, gy0 = 0.0, 0.0
    gx1, gy1 = 8 * cell_mm, 8 * cell_mm
    # Two triangles, normal pointing -Z (outward downward)
    triangles += [
    ...
    for row in range(8):
        for col in range(8):
            ...
            # --- Side walls: only emit exposed bands ---
            # For each neighbor: if it's shorter, emit the band from neighbor_h to h.
            # If no neighbor (grid edge), emit full wall from 0 to h.
            # If neighbor is taller or equal, suppress entirely.

            def wall(p0, p1, p2, p3):
                """Emit two triangles for a quad (p0,p1,p2,p3 in CCW order from outside)."""
                return [[p0, p1, p2], [p0, p2, p3]]

            # Front wall (-Y face): exposed toward row-1
            n_h = heights[row - 1, col] if row > 0 else 0.0
            if h > n_h:
                z_base = n_h
                triangles += wall(
                    np.array([x0, y0, z_base], dtype=np.float32),
                    np.array([x0, y0, h],      dtype=np.float32),
                    np.array([x1, y0, h],      dtype=np.float32),
                    np.array([x1, y0, z_base], dtype=np.float32),
                )

            # Back wall (+Y face): exposed toward row+1
            n_h = heights[row + 1, col] if row < 7 else 0.0
            if h > n_h:
                z_base = n_h
                triangles += wall(
                    np.array([x1, y1, z_base], dtype=np.float32),
                    np.array([x1, y1, h],      dtype=np.float32),
                    np.array([x0, y1, h],      dtype=np.float32),
                    np.array([x0, y1, z_base], dtype=np.float32),
                )

            # Left wall (-X face): exposed toward col-1
            ...
            # Right wall (+X face): exposed toward col+1
